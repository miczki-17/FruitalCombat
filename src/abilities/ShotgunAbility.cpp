...
namespace game::components
{
    namespace
    {
        ...
        const sf::Color PELLET_COLOR(45, 25, 15);
    }
    ...
    void ShotgunAbility::update(float deltaTime)
    {
        ...
        const sf::Vector2f& ownerVelocity)
    {
        ...
        spawnPellets(origin, direction, ownerVelocity);
        resetCooldown();
    ...
    {
        sf::Vector2f direction = targetPosition - origin;
        ...
        const sf::Vector2f& ownerVelocity)
    {
        ...
        const float spreadRadians = spreadAngleDegrees_ * (std::numbers::pi_v<float> / 180.0f);
        const float halfSpread = spreadRadians / 2.0f;

        for (int pelletIndex = 0; pelletIndex < pelletCount_; ++pelletIndex)
        {
            const float angleOffset = (pelletCount_ > 1) ?
                (-halfSpread + (spreadRadians * static_cast<float>(pelletIndex) / (pelletCount_ - 1))) : 0.0f;

            const float finalAngle = baseAngle + angleOffset;
            const sf::Vector2f pelletDirection(std::cos(finalAngle), std::sin(finalAngle));
            const sf::Vector2f spawnPosition = origin + (pelletDirection * PELLET_SPAWN_OFFSET);
            ...
            auto projectile = std::make_unique<game::components::ProjectileComponent>(
                spawnPosition, pelletDirection, sourceName_);

            projectile->addVelocity(ownerVelocity * SHOOTER_VELOCITY_FACTOR);
            projectile->setAppearance(PELLET_RADIUS, PELLET_COLOR);
    ...
    bool ShotgunAbility::isOnCooldown() const { return cooldownTimer_ > 0.0f; }
    void ShotgunAbility::resetCooldown() { cooldownTimer_ = cooldown_; }
}